"""
Parse Frontier routes from the route list.
"""
...
def parse_routes():
    ...
    for line in routes_text.strip().split('\n'):
        ...
        origin_code = AIRPORT_CODES.get(origin_name, origin_name[:3].upper())
        dest_code = AIRPORT_CODES.get(dest_name, dest_name[:3].upper())
        ...
        routes.append({
            ...
            'route_display': f"{origin_name} → {dest_name}",
...
if __name__ == "__main__":
    ...
    for route in routes[:10]:
        print(f"  {route['route_display']} ({route['route_code']})")

    # Save to JSON
    import json
    with open('frontier_routes.json', 'w') as f:
        json.dump(routes, f, indent=2)

    print(f"\n✅ Saved to frontier_routes.json")
